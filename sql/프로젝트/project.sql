...
# 주문 상품 연관 구매
SELECT O.orderNumber, productLine, COUNT(productLine)
	FROM orders O
		INNER JOIN orderDetails D
		ON O.orderNumber = D.orderNumber
        INNER JOIN products P
        ON D.productCode = P.productCode
	GROUP BY O.orderNumber, productLine
	ORDER BY O.orderNumber;
        
# comments가 있는 주문건수
SELECT comments, status, COUNT(comments)
	FROM orders
	WHERE comments IS NOT NULL
    GROUP BY comments, status;
    
SELECT MONTH(orderDate) AS order_month, COUNT(orderNumber)
	FROM orders
    GROUP BY order_month;

SELECT YEAR(orderDate) AS order_year, COUNT(orderNumber)
	FROM orders
    GROUP BY order_year;
...
# 1. 배송완료된 주문 매출을 기준으로 사원의 직원번호, 이름과 성, 누적 매출액 구하시오. (누적 매출액이 가장 높은 5등까지만 정렬)
SELECT employeeNumber, lastName, firstName, SUM(amount)
	FROM employees E
		INNER JOIN customers C
        ON E.employeeNumber = C.salesRepEmployeeNumber
		INNER JOIN (
        SELECT P.customerNumber, amount, status
			FROM payments P
				INNER JOIN orders o
                ON P.customerNumber = O.customerNumber) AS P
		ON C.customerNumber = P.customerNumber
	WHERE status = 'Shipped'
	GROUP BY employeeNumber
    ORDER BY SUM(amount) DESC;

# 2. Payments 기준으로 사원의 직원번호, 이름과 성, 누적 매출액 구하시오. (누적 매출액이 가장 높은 5등까지만 정렬)
SELECT employeeNumber, lastName, firstName, SUM(amount)
	FROM payments P
		INNER JOIN customers C
...
# 받은 돈과 받아야 할 돈 비교해서 신용점수 재조정
SELECT * FROM orderdetails;
SELECT * FROM orders;
SELECT * FROM payments;


#) 각 order, customer 에 따라 지불할 가격
SELECT O.orderNumber, customerNumber, SUM(quantityOrdered * priceEach)
	FROM orders O
		INNER JOIN orderdetails D
        ON O.orderNumber = D.orderNumber
...
#) 각 customer 지불내역 비교
SELECT customerNumber, totalamount, totalpayment, totalamount - totalpayment,
	CASE
		WHEN totalamount = totalpayment THEN 0
		WHEN totalamount > totalpayment THEN 1
        WHEN totalamount < totalpayment THEN 2
	END AS comparison
	FROM (
    SELECT O.customerNumber, SUM(quantityOrdered * priceEach) AS totalamount, P.totalpayment
    FROM orders O
    INNER JOIN orderdetails D ON O.orderNumber = D.orderNumber
    INNER JOIN (
        SELECT customerNumber, SUM(amount) AS totalpayment
        FROM payments
        GROUP BY customerNumber) AS P
	ON O.customerNumber = P.customerNumber
    GROUP BY O.customerNumber) AS summary
    HAVING comparison = 1;


# 구매비용과 신용점수와 비교
SELECT C.customerNumber, totalamount, totalpayment, totalamount - totalpayment AS total, totalpayment / totalamount AS percent, comparison, creditLimit
	FROM customers C
    INNER JOIN(
    SELECT customerNumber, totalamount, totalpayment, totalamount - totalpayment,
		CASE
		...
		ON O.customerNumber = P.customerNumber
		GROUP BY O.customerNumber) AS summary
		HAVING comparison = 1) AS P
	ON C.customerNumber = P.customerNumber
    ORDER BY percent ASC, total DESC;
    
    
SELECT AVG(creditLimit), MAX(creditLimit), Min(creditLimit)
	FROM customers;



# order 금액과 신용한도의 상관관계
SELECT C.customerNumber, COUNT(DISTINCT O.orderNumber) AS orderCount, SUM(quantityOrdered * priceEach) AS orderAmount, creditLimit
	FROM customers C
		INNER JOIN orders O
...
# 재고양에 따른 상품 순위
SELECT YEAR(orderDate) AS order_year, MONTH(orderDate) AS order_month, productName, quantityInStock, SUM(quantityOrdered)
	FROM orders O
		INNER JOIN orderdetails D
        ON O.orderNumber = D.orderNumber
        INNER JOIN products P
        ON D.productCode = P.productCode
	GROUP BY order_year, order_month, productName, quantityInStock;


# 주문양에 따른 제조업체 확인
SELECT productVendor, productName, SUM(quantityInStock) AS Stock, SUM(quantityOrdered) AS orderQ
	FROM products P
		INNER JOIN orderdetails O
        ON P.productCode = O.productCode
	GROUP BY productVendor, productName
    ORDER BY productVendor, Stock, orderQ;


SELECT productVendor, SUM(quantityInStock) AS Stock, SUM(quantityOrdered) AS orderQ
	FROM products P
		INNER JOIN orderdetails O
        ON P.productCode = O.productCode
	GROUP BY productVendor
    ORDER BY Stock, orderQ;


# 직원별 맡는 고객들의 comment
SELECT E.employeeNumber, customerName, comments
	FROM employees E
		INNER JOIN customers C
        ON E.employeeNumber = C.salesRepEmployeeNumber
        INNER JOIN orders O
...
# 월별 수익 그래프
SELECT YEAR(orderDate) AS order_year, MONTH(orderDate) AS order_month, productName, productScale, quantityOrdered, priceEach, quantityOrdered * priceEach AS Hap
	FROM orders O
		INNER JOIN orderdetails D
        ON O.orderNumber = D.orderNumber
        INNER JOIN products P
        ON D.productCode = P.productCode
	WHERE YEAR(orderDate) = 2003 AND MONTH(orderDate) = 10
    ORDER BY Hap DESC;
    

SELECT YEAR(orderDate) AS order_year, MONTH(orderDate) AS order_month, productName, quantityOrdered, priceEach, quantityOrdered * priceEach AS Hap
	FROM orders O
		INNER JOIN orderdetails D
        ON O.orderNumber = D.orderNumber
        INNER JOIN products P
...
SELECT productCode, priceEach, YEAR(orderDate) AS order_year, MONTH(orderDate) AS order_month
	FROM orders O
		INNER JOIN orderdetails D
		ON O.orderNumber = D.orderNumber
	WHERE MONTH(orderDate) BETWEEN 9 AND 10;


# 장바구니에 담긴 ProductLine 연관성 분석
SELECT orderNumber, orderLineNumber, productLine
	FROM orderdetails O
		INNER JOIN products P
...
SELECT O.orderNumber, P.productLine, COUNT(*) AS productLineCount
	FROM orderdetails O
		INNER JOIN products P ON O.productCode = P.productCode
	GROUP BY O.orderNumber, P.productLine
	ORDER BY O.orderNumber, P.productLine;
...
SELECT od.productCode,p.productName,
    ...
    SUM(od.quantityOrdered * (od.priceEach - p.buyPrice)) AS totalProfit -- 수량*(판매가-도매가)
...
########## 제품별 순수익과 총매출액 비교
SELECT od.productCode, p.productName,
    SUM(od.quantityOrdered * (od.priceEach - p.buyPrice)) AS totalProfit,   -- 총마진
    SUM(quantityOrdered * priceEach) AS totalSales,  -- 총매출액
...
use classicmodels;
with first_employ_team as (
   select o.officeCode, e.employeeNumber, sum(quantityOrdered * priceEach) as sum_amount, RANK() OVER (PARTITION BY o.officeCode ORDER BY SUM(quantityOrdered * priceEach) DESC) AS salesRank
   from offices o
   join employees e on o.officeCode = e.officeCode
   join customers c on e.employeeNumber = c.salesRepEmployeeNumber
   join orders od on c.customernumber = od.customernumber
   join orderdetails odt on od.orderNumber = odt.orderNumber
    group by o.officeCode, e.employeeNumber
)
select officeCode, employeeNumber, sum_amount
from first_employ_team
where salesRank = 1
...
# Classic Cars
WITH ordersclassification AS(
	SELECT O.orderNumber,
		MAX(CASE WHEN P.productLine = 'Classic Cars' THEN 1 ELSE 0 END) AS ClassicCars,
		MAX(CASE WHEN P.productLine = 'Motorcycles' THEN 1 ELSE 0 END) AS Motorcycles,
		MAX(CASE WHEN P.productLine = 'Planes' THEN 1 ELSE 0 END) AS Planes,
		MAX(CASE WHEN P.productLine = 'Ships' THEN 1 ELSE 0 END) AS Ships,
		MAX(CASE WHEN P.productLine = 'Trains' THEN 1 ELSE 0 END) AS Trains,
		MAX(CASE WHEN P.productLine = 'Trucks and Buses' THEN 1 ELSE 0 END) AS TrucksAndBuses,
		MAX(CASE WHEN P.productLine = 'Vintage Cars' THEN 1 ELSE 0 END) AS VintageCars
		FROM orderdetails O
			INNER JOIN products P ON O.productCode = P.productCode
...
SELECT
    ...
    SUM(Planes) AS Planes,
    SUM(Ships) AS Ships,
    SUM(Trains) AS Trains,
    SUM(TrucksAndBuses) AS TrucksAndBuses,
    SUM(VintageCars) AS VintageCars
FROM 
...
WITH ordersclassification AS(
	SELECT O.orderNumber,
		...
		ORDER BY O.orderNumber)
SELECT
    SUM(ClassicCars) AS ClassicCars,
    SUM(Planes) AS Planes,
    SUM(Ships) AS Ships,
    SUM(Trains) AS Trains,
    SUM(TrucksAndBuses) AS TrucksAndBuses,
...
# 4조
# 1. 각 사원의 총 매출을 계산하고, 첫번째 주문을 받은 날짜와 마지막 주문을 받은 날짜의 차이를 이용한 근무기간을 통해 일 평균 매출을 구하시오.


# 2. 각 회사의 총 매출을 계산한 후, 해당 회사가 속한 국가로 통합하여 국가별 총 매출